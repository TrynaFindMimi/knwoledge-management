...
| k6 | Perf developer-friendly (búsqueda <3s con 1000 docs, 10 concurrentes) |
| **JMeter** | **Perf/carga y estrés (15 usuarios concurrentes, 1000+ docs, throughput, latencia p95 <3s búsqueda / <2s operaciones, pruebas de endurance)** |
| OWASP ZAP | Pen-test |
...
| **S4** | LangChain (retrieval+rerank), k6 + **JMeter** (plan base), Socket.io, AntD Drawer/List/Input.Search | Búsqueda <3s p95, chat con citas linkeadas + memoria `¿y qué más de Mamani?`, dataset 100 queries ≥85%, thread group JMeter inicial validado |
...
| — Perf (operaciones <2s, búsqueda 10 concurrentes/1000 docs) | | | **k6 + JMeter** (reporte throughput, latencia p95, errores 0%, 15 usuarios sostenidos) |
| — Carga/estrés (endurance 30 min, ramp-up, picos) | | | **JMeter** (Thread Group 15 users, assertions `busqueda_p95 < 3000ms`) |
